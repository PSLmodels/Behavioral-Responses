...
#         (3) builds local behavioral-responses=0.0.0 package (conda build)
#         (4) installs local behavioral-responses=0.0.0 package (conda install)
...
if [ $? -eq 1 ]; then
    ...
    echo "==> Continuing to build new behavioral-responses package"
fi
...
# build behavioral-responses conda package for this version of Python
NOHASH=--old-build-string
pversion=3.6
conda build $NOHASH --python $pversion . 2>&1 | awk '$1~/BUILD/||$1~/TEST/'

# install behavioral-responses conda package
echo "INSTALLATION..."
conda install behavioral-responses=0.0.0 --use-local --yes 2>&1 > /dev/null
...
# NOTE: conda install -c local behavioral-responses=0.0.0 --yes 2>&1 > /dev/null
...
rm -rf dist/*
rmdir dist/
rm -rf behavioral-responses.egg-info/*
rmdir behavioral-responses.egg-info/
popd > /dev/null
